class CreateMarket < ActiveRecord::Migration
  def up
    ...
    Market.create_translation_table! :title => :string, :description => :text,
      :address => :string, :full_name => :string, :our_benefits => :text, :our_brands => :text
  end
  def down
      drop_table :markets
    Market.drop_translation_table!
  end
end
